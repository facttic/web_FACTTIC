 ...
 */
function aReferencia(ref: Ref<{ _id: string; nombre?: string }> | undefined | null): Dominio.Referencia | null {
...
function aReferencias(
  ...
  return (refs ?? []).map(aReferencia).filter((r): r is Dominio.Referencia => r !== null)
}
...
export function aServicio(api: Api.Servicio): Dominio.Servicio {
  return {
    ...
    subservicios: (api.subservicios ?? [])
      .map((s) => ({ nombre: texto(s.nombre) ?? '', descripcion: texto(s.descripcion) }))
      .filter((s) => s.nombre),
  }
}
...
export function aCooperativa(api: Api.Cooperativa): Dominio.Cooperativa {
  return {
    ...
    ubicacion: api.ubicacion?.lat != null && api.ubicacion?.lng != null ? api.ubicacion : null,
