...
export const API_URL = process.env.FACTTIC_API_URL ?? 'https://facttic-web.it10.com.ar'
...
async function login(): Promise<CachedToken> {
  ...
  if (!res.ok) {
    throw new Error(`Login contra la API falló: ${res.status} ${res.statusText}`)
  }

  const { access_token: value } = (await res.json()) as { access_token?: string }
