...
function Portada({ proyecto, className }: { proyecto: Proyecto; className?: string }) {
  if (!proyecto.portada) {
...
export function CardProyecto({
  ...
  return (
    <Link
      ...
      className={cn(
        'group flex flex-col overflow-hidden rounded-xl border border-borde bg-superficie',
  ...
  return (
    ...
    >
      ...
      <div className="space-y-5 p-6">
        ...
        {proyecto.desafio ? (
          <p className="text-p2 line-clamp-3 text-blanco/60">{proyecto.desafio}</p>
        ) : null}

        <div className="flex flex-wrap items-center gap-3">
          {proyecto.cliente ? <Chip>{proyecto.cliente.nombre}</Chip> : null}
          {proyecto.sector ? <ChipSector nombre={proyecto.sector.nombre} /> : null}
...
export function FilaProyecto({ proyecto }: { proyecto: Proyecto }) {
  return (
    <Link
      href={`/proyectos/${proyecto.slug}`}
      className="group flex flex-col gap-4 border-b border-dashed border-borde py-5 transition-colors hover:bg-superficie/50 md:flex-row md:items-center md:gap-6"
    >
      ...
      <svg
        ...
        fill="none"
        aria-hidden
        className="hidden size-5 shrink-0 transition-transform group-hover:translate-x-1 md:block"
      >
        <path
